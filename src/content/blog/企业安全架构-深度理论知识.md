---
title: '企业安全架构深度理论知识'
description: '全面解析企业安全架构核心理论：零信任架构、RBAC/ABAC权限模型、OAuth/OIDC、LLM安全、数据脱敏与PII保护等专家级安全知识体系。'
pubDate: 2026-01-30
tags: ['安全']
---
...
> **学习深度**: ⭐⭐⭐⭐⭐
> **文档类型**: 纯理论知识（无代码实践）
> **权威参考**: NIST、OWASP、IETF、OpenID Foundation
...
4. 短期令牌 → 用户 → 访问资源
```
- **关键技术**: SAML 2.0 / OAuth 2.0 + OIDC；OAuth 2.1 仍处于 IETF Internet-Draft 阶段时，应按“工作草案”引用，并结合已发布 RFC 与安全最佳实践落地
...
**标准状态说明**: 截至本文更新时，OAuth 2.1 仍是 IETF OAuth Working Group 的 Internet-Draft，不应当在架构评审中把它描述为已发布 RFC。工程落地宜以 OAuth 2.0、PKCE、Bearer Token、OAuth 2.0 Security BCP、mTLS/DPoP 等已发布规范和最佳实践为基线，再跟踪 OAuth 2.1 草案的收敛方向。
...
**OAuth 2.1 草案体现的安全方向**:
- 授权码流程默认配合 PKCE，降低授权码拦截风险
- 不再推荐隐式流 (Implicit Flow) 和资源所有者密码凭证模式
- Refresh Token 应采用轮换、重用检测或发送方约束，避免长期 bearer 凭证失控
- 所有授权和资源访问链路必须使用 TLS，并精确校验重定向 URI、issuer、audience 等边界
...
| **访问令牌** | 1小时 | 访问资源 | 否 | 中 |
| **刷新令牌** | 按风险配置 | 获取新访问令牌 | 是 | 高（需轮换、撤销、重用检测或发送方约束） |

#### OpenID Connect (OIDC)

**定位**: OIDC 是建立在 OAuth 2.0 之上的身份认证层；OAuth 2.1 草案不改变 OIDC 的基本定位。
...
### 3.4 OWASP Top 10 for LLM / GenAI 2025

OWASP GenAI Security Project 在 2025 版中重新组织了 LLM 应用风险，覆盖传统 LLM 应用、RAG、向量数据库、供应链和 Agentic AI。下列编号按 OWASP 2025 版校准；旧版中的 Training Data Poisoning、Insecure Output Handling、Model DoS 等概念仍重要，但名称和排序已调整。
...
**风险**: LLM 或其应用上下文泄露敏感信息，包括个人信息、业务秘密、凭证、其他用户会话内容，以及被错误放入提示词或检索上下文中的内部资料。

**防御**:
- 输入、检索、日志、输出全链路做数据分类与脱敏
- 禁止把密钥、内部策略、完整系统提示词当作可依赖的访问控制边界
- 对 RAG 检索结果和工具返回值做权限过滤，避免越权上下文进入模型

#### LLM03:2025 Supply Chain

**风险**: 模型、数据集、插件、Agent 工具、依赖包、镜像、提示词模板或第三方 API 被污染或替换，导致后门、数据泄露或不受控行为。

**防御**:
- 维护模型、数据、插件和依赖的清单与版本来源
- 使用签名、校验、制品仓库准入和漏洞扫描
- 对第三方模型/API 做隔离、最小权限和退出方案设计

#### LLM04:2025 Data and Model Poisoning

**风险**: 恶意数据进入预训练、微调、RAG 知识库、向量索引或反馈闭环，影响模型输出或触发后门行为。

**攻击方式**:
- 投毒公开数据源、企业知识库、客服反馈或埋点日志
- 在向量库中植入高相似度恶意文档
- 利用自动反馈训练放大错误行为
...
**风险**: LLM 或 Agent 被授予过多工具、权限、自动执行能力或持久凭证，导致越权操作、破坏性操作或被 Prompt Injection 间接操控。
...
**风险**: 系统提示词泄露导致业务逻辑、策略、内部工具说明或敏感上下文暴露。系统提示词泄露本身不必然等同于系统被攻破，但不能把提示词当作秘密存储或强安全边界。

**防御**:
- 不在系统提示词中保存密钥、固定口令、内部访问令牌或完整风控规则
- 将真实授权放在网关、策略引擎和后端服务中执行
- 对提示词、工具描述和检索上下文做最小化暴露

#### LLM08:2025 Vector and Embedding Weaknesses

**风险**: 向量库、Embedding、RAG 检索链路存在越权检索、索引投毒、相似度碰撞、跨租户泄露或删除不彻底等问题。

**防御**:
- 向量索引按租户、数据级别和访问策略隔离
- 检索前后都做权限过滤，而不是只依赖相似度
- 对索引更新、删除、回滚和审计建立可验证流程
...
**风险**: 攻击者通过超长输入、递归工具调用、批量请求、高成本模型路由或复杂检索消耗 token、算力、预算和后端资源，造成拒绝服务或成本失控。

**防御**:
- 对 token、调用次数、工具链深度、并发、预算做多维配额
- 对 Agent 循环调用设置终止条件和人工介入阈值
- 监控异常成本、异常延迟和资源消耗模式
...
安全架构中的数据分类、最小化、留存、审计、跨境传输和第三方处理，应与企业所在行业和地域的合规要求对齐。中国场景通常需要关注《个人信息保护法》(PIPL)、数据出境安全评估/标准合同/个人信息保护认证路径，以及网络安全等级保护相关标准（如 GB/T 22239-2019）。

本文只讨论工程控制点，不构成法律意见。工程上可优先落实以下边界：

- 明确个人信息、敏感个人信息、重要数据、业务秘密和公开数据的分级口径
- 在跨境调用模型、日志平台、SaaS IAM、RAG 知识库或海外 API 前，识别是否存在个人信息或重要数据出境
- 用数据最小化、脱敏、加密、访问审批、审计留痕和供应商评估降低暴露面
- 将等保、行业监管和公司内部制度转化为可执行的身份、网络、日志、备份、应急和变更控制要求
...
graph TB
    ...
    subgraph Data_Mask[数据脱敏方案]
        ...
        Compliance[合规要求<br/>→ GDPR/PIPL/等保语境<br/>分类分级]
    end
...
- **OAuth 2.1 Authorization Framework (Internet-Draft)**
  https://www.ietf.org/archive/id/draft-ietf-oauth-v2-1-13.html

- **RFC 9700: Best Current Practice for OAuth 2.0 Security**
  https://www.ietf.org/rfc/rfc9700.html

- **RFC 8705: OAuth 2.0 Mutual-TLS Client Authentication and Certificate-Bound Access Tokens**
  https://www.ietf.org/rfc/rfc8705.html
