...
description: 'IAM系统完整解析：JWT/Opaque Token机制、网关认证鉴权架构、RBAC/ABAC权限模型、AI Agent短期凭证与身份管理及主流IAM方案深度对比。'
pubDate: 2026-01-30
tags: ['安全']
---
...
1. 网关入口先清洗/删除所有客户端提交的 `X-User-*`、`X-Tenant-*`、`X-Agent-*` 等内部上下文 Header，防止伪造。
2. 网关到后端使用 mTLS、服务网格身份或等价网络隔离，后端只接受来自可信网关/sidecar 的流量。
3. 用户上下文 Header 需要签名或封装为内部 JWT，包含 `iss`、`aud`、`iat`、`exp`、`jti`，后端验证签名和短过期时间。
4. Header 中只传递最小必要字段，不传身份证号、手机号、完整邮箱、原始 Token 等敏感数据。
5. 后端对高风险操作仍应调用策略服务或验证内部令牌，不能把明文 Header 当作最终授权事实。
...
- Token 必须包含 `aud`、`scope`、`jti`、`exp`、委托关系和资源边界，权限变更应可即时撤销或快速收敛
...
6. 记录完整的审计日志
7. AI Agent使用最小权限原则
8. 避免长期或永久Agent bearer token；采用短期、可撤销、可轮换、发送方约束的凭证
9. 客户端传入的用户上下文Header必须在网关入口清洗，禁止后端直接信任外部Header
...
### 标准规范
- [RFC 7519: JWT](https://www.ietf.org/rfc/rfc7519.html)
- [RFC 7662: OAuth 2.0 Token Introspection](https://www.ietf.org/rfc/rfc7662.html)
- [RFC 8693: OAuth 2.0 Token Exchange](https://www.ietf.org/rfc/rfc8693.html)
- [RFC 8705: OAuth 2.0 Mutual-TLS Client Authentication and Certificate-Bound Access Tokens](https://www.ietf.org/rfc/rfc8705.html)
- [RFC 9449: OAuth 2.0 Demonstrating Proof of Possession (DPoP)](https://www.ietf.org/rfc/rfc9449.html)
- [RFC 9700: Best Current Practice for OAuth 2.0 Security](https://www.ietf.org/rfc/rfc9700.html)
- [OAuth 2.1 Authorization Framework (Internet-Draft)](https://www.ietf.org/archive/id/draft-ietf-oauth-v2-1-13.html)
